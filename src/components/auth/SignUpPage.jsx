...
function StrengthMeter({ password }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      {password && (
        <span style={{ fontFamily: 'Syne', fontSize: '0.75rem', color: colors[count] }}>
          {labels[count]}
        </span>
      )}
      <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 3 }}>
        {[
          ...
          <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontFamily: 'Syne', fontSize: '0.72rem', color: checks[key] ? '#00FF88' : '#555' }}>
              {label}
            </span>
          </div>
        ))}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        {icon && (
          <span style={{ position: 'absolute', left: 14, color: '#555', display: 'flex', zIndex: 1 }}>{icon}</span>
        )}
        <div style={{ width: '100%' }}>{children}</div>
        {suffix && (
          <span style={{ position: 'absolute', right: 14, color: '#888', display: 'flex', zIndex: 1, cursor: 'pointer' }}>{suffix}</span>
        )}
      </div>
      {error && (
...
export default function SignUpPage() {
  ...
  const canSubmit = form.name.length >= 2 && emailValid && /^[6-9][0-9]{9}$/.test(form.mobile)
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative', background: '#0A0A0A' }}>
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: 'Orbitron', fontSize: '1.3rem', fontWeight: 700, color: '#F5F5F5', marginBottom: 6 }}>
            Create Your Account
          </h1>
          <p style={{ fontFamily: 'Syne', fontSize: '0.82rem', color: '#666' }}>
            Join India's smartest statistical trading intelligence
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <InputWrapper icon={<User size={16} />} label="Full Name" error={errors.name}>
            <input
              id="signup-name"
              className={`artha-input ${errors.name ? 'error' : ''}`}
          ...
          >
            <input
              id="signup-email"
              className={`artha-input ${errors.email ? 'error' : emailValid ? 'success' : ''}`}
          ...
          {/* Mobile */}
          <InputWrapper label="Mobile Number (India)" error={errors.mobile}>
            <div style={{ display: 'flex', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,107,0,0.2)', borderRadius: 8, padding: '0 12px', height: 52, whiteSpace: 'nowrap' }}>
                <span>🇮🇳</span>
                <span style={{ fontFamily: 'JetBrains Mono', color: '#888', fontSize: '0.9rem' }}>+91</span>
              </div>
              <input
                id="signup-mobile"
                className={`artha-input ${errors.mobile ? 'error' : ''}`}
                style={{ flex: 1 }}
          ...
          >
            <input
              id="signup-password"
              className={`artha-input ${errors.password ? 'error' : ''}`}
              style={{ paddingLeft: 40, paddingRight: 40 }}
          ...
          <InputWrapper
            ...
            suffix={
              <button type="button" onClick={() => setShowConfirm(!showConfirm)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#888' }}>
          ...
          >
            <input
              id="signup-confirm"
              className={`artha-input ${passMismatch ? 'error' : passMatch ? 'success' : ''}`}
              style={{ paddingLeft: 40, paddingRight: 40 }}
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <button
              ...
              style={{
                width: 20, height: 20, borderRadius: 4, border: `1px solid ${agreed ? '#FF6B00' : 'rgba(255,107,0,0.3)'}`,
                background: agreed ? '#FF6B00' : 'transparent', cursor: 'pointer', flexShrink: 0, marginTop: 2,
            ...
            <span style={{ fontFamily: 'Syne', fontSize: '0.8rem', color: '#888', lineHeight: 1.5 }}>
              I agree to{' '}
              <span style={{ color: '#FF6B00', cursor: 'pointer' }}>Terms & Conditions</span>{' '}
              and{' '}
          ...
          >
            {loading ? (
              <><div className="spinner" /><span>Creating your account...</span></>
            ) : '🚀 CREATE MY ARTHA ACCOUNT'}
          </button>
        </form>
